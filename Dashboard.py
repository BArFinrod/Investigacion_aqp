...
#%%
fig2 = go.Figure([go.Scatter(x=x[nterms:], y=y[nterms:], mode='markers', hovertext=[f"<b>{dfciiu_aqp1.loc[row, 'real_name']} </b> <br>Total de ventas en millones de soles (2015): {dfciiu_aqp1.loc[row, 'size_sales_1m']} <br>Número de empresas (2015): {dfciiu_aqp1.loc[row, 'n_empresas']}" for row in dfciiu_aqp1.index], marker=dict(size=dfciiu_aqp1['size_sales_1m']/3,color='steelblue'),
                            hoverinfo='text',name='Sectores industriales en Arequipa'),
                  go.Scatter(x=x[:nterms], y=y[:nterms], mode='markers', text=dfcen['name'], marker={'color':'green','size':20}, hovertemplate ='<b>%{text}</b>', name='Centros de investigación en Arequipa <br> autorizados por CONCYTEC (Ley 30309)')])
# fig2.show()
#%%
...
st.markdown("En el siguiente archivo se muestran los detalles. Si tienen problemas en visualizarlo, prueba con Chrome o Firefox.")
